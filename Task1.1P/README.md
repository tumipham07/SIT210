...
After that, the loop() calls two custom functions: porchLight() and hallwayLight(). The porchLight() function checks if the porch light is on (porchState = true) AND if 30 seconds (30000 milliseconds) have passed since the porch timer was set. If both conditions are true, it turns off the porch light and updates porchState to false. It is similar to the hallwayLight() function, checking if 60 seconds (60000 milliseconds) have passed before turning off the hallway light.
